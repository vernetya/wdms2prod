use std::str::FromStr;

#[derive(Debug)]
pub enum SemVerToken {
    Some(u16),
    Any,
}

#[derive(Debug)]
pub struct SemVer {
    pub major: SemVerToken,
    pub minor: SemVerToken,
    pub patch: SemVerToken,
}

impl FromStr for SemVerToken {
    type Err = String;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "*" => Ok(Self::Any),
            _ => match s.parse::<u16>() {
                Ok(v) => Ok(Self::Some(v)),
                _ => Err(format!("{} is invalid", s)),
            },
        }
    }
}

impl SemVerToken {
    fn value(&self) -> Option<u16> {
        match self {
            Self::Any => None,
            Self::Some(v) => Some(*v),
        }
    }

    // fn to_regex_group(&self) -> String {
    //     match self {
    //         Self::Any => r"(\d)+".to_string(),
    //         Self::Some(v) => v.to_string(),
    //     }
    // }
}

impl ToString for SemVerToken {
    fn to_string(&self) -> String {
        match self {
            Self::Any => "*".to_string(),
            Self::Some(v) => v.to_string(),
        }
    }
}

// impl SemVer {
//     fn to_regex_group(&self) -> String {
//         format!(
//             "{}.{}.{}",
//             self.major.to_regex_group(),
//             self.minor.to_regex_group(),
//             self.patch.to_regex_group()
//         )
//     }
// }

impl FromStr for SemVer {
    type Err = String;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // note: this only works since there's only ASCII char
        if let Some(p1) = s.find('.') {
            if let Some(p2) = s[p1 + 1..].find('.') {
                let major = s[0..p1].parse::<SemVerToken>()?;
                let minor = s[p1 + 1..p1 + 1 + p2].parse::<SemVerToken>()?;
                let patch = s[p1 + 2 + p2..].parse::<SemVerToken>()?;
                return Ok(SemVer {
                    major,
                    minor,
                    patch,
                });
            }
        }

        Err(format!("{} invalid semver", s))

        // using regex is 50x slower
        // lazy_static! {
        //     static ref RE_SEMVER_WILDCARD: Regex = Regex::new(
        //         r"^(?P<major>(?:(\d)+|\*)).(?P<minor>(?:(\d)+|\*)).(?P<patch>(?:(\d)+|\*))$"
        //     ).expect("fatal invalid static regex");
        // }
        // match RE_SEMVER_WILDCARD.captures(s) {
        //     None => Err(format!("{} is an invalid semver", s)),
        //     Some(captures) =>
        //         Ok(Self{
        //             major: SemVerToken::from_str(captures.name("major").ok_or("invalid semver".to_string())?.as_str())?,
        //             minor: SemVerToken::from_str(captures.name("minor").ok_or("invalid semver".to_string())?.as_str())?,
        //             patch: SemVerToken::from_str(captures.name("patch").ok_or("invalid semver".to_string())?.as_str())?
        //         })
        // }
    }
}

// macro_rules! sem_ver {
//     ($a:expr,$b:expr,$c:expr) => {
//         SemVer {
//             major: SemVerToken::Some($a),
//             minor: SemVerToken::Some($b),
//             patch: SemVerToken::Some($c),
//         }
//     };
//     ($a:expr,$b:expr,_) => {
//         SemVer {
//             major: SemVerToken::Some($a),
//             minor: SemVerToken::Some($b),
//             patch: SemVerToken::Any,
//         }
//     };
//     ($a:expr,_,$c:expr) => {
//         SemVer {
//             major: SemVerToken::Some($a),
//             minor: SemVerToken::Any,
//             patch: SemVerToken::Some($c),
//         }
//     };
//     (_,$b:expr,$c:expr) => {
//         SemVer {
//             major: SemVerToken::Any,
//             minor: SemVerToken::Some($b),
//             patch: SemVerToken::Some($c),
//         }
//     };
//     ($a:expr,_,_) => {
//         SemVer {
//             major: SemVerToken::Some($a),
//             minor: SemVerToken::Any,
//             patch: SemVerToken::Any,
//         }
//     };
//     (_,$b:expr,_) => {
//         SemVer {
//             major: SemVerToken::Any,
//             minor: SemVerToken::Some($b),
//             patch: SemVerToken::Any,
//         }
//     };
//     (_,_,$c:expr) => {
//         SemVer {
//             major: SemVerToken::Any,
//             minor: SemVerToken::Any,
//             patch: SemVerToken::Some($c),
//         }
//     };
//     (_,_,_) => {
//         SemVer {
//             major: SemVerToken::Any,
//             minor: SemVerToken::Any,
//             patch: SemVerToken::Any,
//         }
//     };
// }
// pub(crate) use sem_ver;

impl SemVerToken {
    fn is_match(&self, t: &SemVerToken) -> bool {
        match self {
            SemVerToken::Any => true,
            SemVerToken::Some(left) => match t {
                SemVerToken::Some(right) => left == right,
                _ => false,
            },
        }
    }
}

impl SemVer {
    pub fn any() -> Self {
        Self {
            major: SemVerToken::Any,
            minor: SemVerToken::Any,
            patch: SemVerToken::Any
        }
    }

    pub fn any_minor(major: u16) -> Self {
        Self {
            major: SemVerToken::Some(major),
            minor: SemVerToken::Any,
            patch: SemVerToken::Any
        }
    }
    pub fn any_patch(major: u16, minor: u16) -> Self {
        Self {
            major: SemVerToken::Some(major),
            minor: SemVerToken::Some(minor),
            patch: SemVerToken::Any
        }
    }

    pub fn is_match(&self, s: &str) -> bool {
        match s.parse::<SemVer>() {
            Err(_) => false,
            Ok(other) => self.is_match_other(&other),
        }
    }

    pub fn is_match_other(&self, other: &Self) -> bool {
        self.major.is_match(&other.major)
            && self.minor.is_match(&other.minor)
            && self.patch.is_match(&other.patch)
    }
}

impl ToString for SemVer {
    fn to_string(&self) -> String {
        format!(
            "{}.{}.{}",
            self.major.to_string(),
            self.minor.to_string(),
            self.patch.to_string()
        )
    }
}

#[cfg(test)]
mod test {
    #[allow(unused_imports)]
    use super::*;
    // use ::test::Bencher;

    #[test]
    fn test_parse_semver() {
        let v = SemVer::from_str("1.0.5").unwrap();
        assert_eq!(v.major.value().unwrap(), 1);
        assert_eq!(v.minor.value().unwrap(), 0);
        assert_eq!(v.patch.value().unwrap(), 5);
        assert_eq!(v.to_string(), "1.0.5");

        let v = SemVer::from_str("1.0.*").unwrap();
        assert_eq!(v.major.value().unwrap(), 1);
        assert_eq!(v.minor.value().unwrap(), 0);
        assert!(v.patch.value().is_none());
        assert_eq!(v.to_string(), "1.0.*");

        let v = SemVer::from_str("54.*.1235").unwrap();
        assert_eq!(v.major.value().unwrap(), 54);
        assert!(v.minor.value().is_none());
        assert_eq!(v.patch.value().unwrap(), 1235);
        assert_eq!(v.to_string(), "54.*.1235");
    }

    #[test]
    fn test_match_semver() {
        let v = SemVer::from_str("1.0.5").unwrap();
        assert!(v.is_match("1.0.5"));
        assert!(!v.is_match("1.0.*"));
        assert!(!v.is_match("1.0.42"));
        assert!(!v.is_match("1.1.5"));
        assert!(!v.is_match("2.0.5"));

        let v = SemVer::from_str("1.0.*").unwrap();
        assert!(v.is_match("1.0.5"));
        assert!(v.is_match("1.0.*"));
        assert!(v.is_match("1.0.42"));
        assert!(!v.is_match("1.1.5"));
        assert!(!v.is_match("2.0.5"));

        let v = SemVer::from_str("1.*.*").unwrap();
        assert!(v.is_match("1.0.5"));
        assert!(v.is_match("1.0.42"));
        assert!(v.is_match("1.1.5"));
        assert!(!v.is_match("2.0.5"));

        let v = SemVer::from_str("*.*.*").unwrap();
        assert!(v.is_match("1.0.5"));
        assert!(v.is_match("1.0.42"));
        assert!(v.is_match("1.1.5"));
        assert!(v.is_match("2.0.5"));
    }

    // #[test]
    // fn test_macro_semver() {
    //     assert_eq!(sem_ver!(1, 0, 5).to_string(), "1.0.5");
    //     assert_eq!(sem_ver!(1, 0, _).to_string(), "1.0.*");
    //     assert_eq!(sem_ver!(1, _, 5).to_string(), "1.*.5");
    //     assert_eq!(sem_ver!(_, 0, 5).to_string(), "*.0.5");
    //     assert_eq!(sem_ver!(1, _, _).to_string(), "1.*.*");
    //     assert_eq!(sem_ver!(_, 0, _).to_string(), "*.0.*");
    //     assert_eq!(sem_ver!(_, _, 5).to_string(), "*.*.5");
    //     assert_eq!(sem_ver!(_, _, _).to_string(), "*.*.*");
    // }

    // #[bench]
    // fn bench_parse(b: &mut Bencher) {
    //     // b.iter(|| get_ss("1.0.5"));
    //     b.iter(|| "1.0.5".parse::<SemVer>());
    // }
    //
    // #[bench]
    // fn bench_is_match(b: &mut Bencher) {
    //     let v = "1.0.*".parse::<SemVer>().unwrap();
    //     b.iter(|| v.is_match("1.0.42"));
    // }
}
