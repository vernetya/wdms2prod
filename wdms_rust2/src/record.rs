...
use crate::validation::{load_schema_validator, validation_error_descr};
...
impl Record {
    pub fn new_unchecked(instance: Value) -> Result<Self, (Value, String)> {
    ...
    pub fn new_checked(instance: Value) -> Result<Self, (Value, Vec<String>)> {
        if !instance.is_object() {
    ...
    pub fn as_map(&self) -> RecordMap {
        RecordMap{map: self.value.as_object().unwrap()}
    }
...
mod test {
    ...
    fn test_load_record() {
        ...
        let record = Record::new_unchecked(document).unwrap();
        println!("kind => {}", record.as_map().kind().unwrap());
    }
    ...
    fn test_validate_record() {
        let f = std::fs::File::open("res/examples/wellLog_v3_120.json").unwrap();
        let document: Value = serde_json::from_reader(f).unwrap();
        let record = Record::new_checked(document);
        match record {
            Ok(_) => {}
            Err((_, err)) => {
    ...
    //     b.iter(|| EntityKind::parse("osdu:wks:work-product-component--WellLog:1.2.50"));
    // }
    //
    // #[bench]
